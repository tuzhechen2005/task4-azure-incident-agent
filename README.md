...
要启用真实 DeepSeek V4 Pro 分析，在本地 `.env` 中设置以下内容（不要提交真实值）：
...
应用通过 DeepSeek 官方 API 直接调用 `deepseek-v4-pro`，无需 Azure 订阅、Endpoint 或模型部署。配置完整时 `/api/health` 的 `analysis_mode` 为 `deepseek`；配置缺失、请求失败、限流、超时或输出校验失败时，事件会自动保存为安全的 `FALLBACK` 分析。
...
| `LLM_BASE_URL` | `https://api.deepseek.com` | DeepSeek 官方 OpenAI 兼容 API 地址 |
| `LLM_MODEL` | `deepseek-v4-pro` | DeepSeek 模型标识 |
...
系统提示词定义四级严重程度、禁止臆测规则、公共状态局限、简体中文输出要求和严格 JSON 契约。经过校验的 `AgentInput` 会被确定性序列化，并放入明确的不可信数据边界；订阅源正文中的指令不会被执行。DeepSeek Responses API 被要求按严格 JSON Schema 返回一个对象，随后仍会经过 Pydantic 校验并核对事件 ID。时间、来源和模型元数据由应用代码控制。
...
- 仓库包含 DeepSeek Responses API 适配器；真实可用性取决于调用者自己的 DeepSeek API Key、账户余额、模型权限、配额和网络。
