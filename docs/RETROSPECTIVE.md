...
项目有意保持本地化和简单化：一个 Python 进程、一个 SQLite 数据库、原生 HTML／CSS／JavaScript。DeepSeek API Key 可启用真实智能体，未提供凭据时仍可使用完整的本地备用流程。
...
模型提示词是确定性的，将订阅源文本视为不可信数据，定义所有严重级别，禁止臆测，要求简体中文内容，并严格限定为一个符合结构的 JSON 对象。真实路径通过 DeepSeek Responses API 的严格 JSON Schema 请求 `deepseek-v4-pro`；应用随后再次校验结构和事件 ID。任何失败都会生成完整备用结果，而不是保留部分模型输出。备用结果会明确表达不确定性，并要求运维人员核实真实本地影响。
...
- 真实智能体需要调用者自己的 DeepSeek API Key、账户余额、模型权限、配额和网络；这些外部条件不进入默认离线测试。
...
1. 增加可选的多提供商适配层，并保留 DeepSeek API Key 与 fallback 路径。
